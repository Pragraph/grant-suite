...
>(({ className, children, ...props }, ref) => (
  <RadixSelect.Trigger
    ref={ref}
    className={cn(
      "flex h-9 w-full items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-foreground shadow-xs transition-colors duration-fast placeholder:text-gray-400 focus:border-[#4F7DF3] focus:outline-none focus:ring-2 focus:ring-[#4F7DF3]/20 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <RadixSelect.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-md py-1.5 pl-8 pr-2 text-sm text-foreground outline-none transition-colors focus:bg-gray-50 focus:text-foreground data-[state=checked]:bg-[#F0F4FF] data-[state=checked]:text-[#4F7DF3] data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
